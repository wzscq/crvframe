...
import { modiData,removeErrorField } from '../../../../../../redux/tmpDataSlice';
import {
    FRAME_MESSAGE_TYPE,
    CASCADE_TYPE,
    FORM_LABEL_POS
...
const getManyToOneValueFunc=(optionLabel)=>{
...
export default function SingleSelectForManyToOne({dataPath,control,field,sendMessageToParent,labelPos}){
    ...
    const {updatedValue,valueError}=useSelector(state=>selectValue(state.tmpdata,dataPath,field.field));

    const selectCascadeValue = useMemo(makeCascadeSelector, [dataPath,control,field]);
    const cascadeParentValue=useSelector(state=>selectCascadeValue(state.tmpdata,dataPath,field.field,control.cascade));
    ...
    useEffect(()=>{
        const queryResponse=(event)=>{
            ...
            if(type===FRAME_MESSAGE_TYPE.QUERY_RESPONSE&&
                ...
                /*if(updatedValue===undefined&&data.list.length>0&&data.list[0]?.id!==undefined){
                    onChange(data.list[0]?.id);
                }*/
            }
        }
        ...
        if(updatedValue===undefined&&control.defaultValue!==undefined){
            const funStr='"use strict";'+
                     'return (function(moment){ '+
                        ...
                        '}'+
                     '})';
            const defaultValue=Function(funStr)()();
